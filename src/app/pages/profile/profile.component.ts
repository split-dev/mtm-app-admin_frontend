...
import {catchError, concatMap, debounceTime, takeWhile} from 'rxjs/operators';
...
export class ProfileComponent {
  ...
  errorMessage: string = 'Invalid value. Must be in range'
  errorMessageMeasurements: string = ''
  isValueValidJMeasurements: number[] = [-1];
  ...
  saveChanges(): void {
    ...
    this.metafieldsService.updateCustomerMetafieldsProfileCompleted({
      owner_id: this.customerId,
      flag: this.profile_completed
    }).subscribe((res) => {
  ...
  onInputBlur(ev: any, i: number, j: number, type: string, ms: string, gender: string) {
    let value = ev.trim().replace(',', '.');
    console.log('value',value)
    if (value !== '0' || ms === 'front_waist_height' || ms === 'back_waist_height') {
      ...
      // Перевіряємо, чи є букви
      const hasLetters = /[a-zA-Z]/.test(value);
      if (hasLetters) {
        this.isValueValidJ.push(j)
        this.isValueValidI.push(i)
        this.isValueValid = 1
        this.errorMessage = 'Invalid value. Only numbers are allowed.'
      } else {
        console.log('objMinAndMax', objMinAndMax)
        let extractedNumber = parseFloat(value);
        extractedNumber = parseFloat(extractedNumber.toFixed(2));
        if (extractedNumber < objMinAndMax.min || (extractedNumber > objMinAndMax.max && extractedNumber !== objMinAndMax.max)) {
          this.isValueValid = 1
          this.isValueValidJ.push(j)
          this.isValueValidI.push(i)
          this.errorMessage = 'Invalid value. Must be in range'
        } else {
  ...
  removeValidationErrorMeasurements(j: number) {
    const indexToRemoveJ = this.isValueValidJMeasurements.indexOf(j);
    if (indexToRemoveJ !== -1) {
      this.isValueValidJMeasurements.splice(indexToRemoveJ, 1);
    }

  }
  updateBodyMeasurementsMetafield(ev: any, group: any, ms: any,j:number) {
    const val = ev.length > 0 ? ev : '0';

    let value = val.replace(',', '.');
    console.log('value',value)
    this.removeValidationErrorMeasurements(j)
    if (value !== '0') {
      // Перевіряємо, чи є букви
      const hasLetters = /[a-zA-Z]/.test(value);
      if (hasLetters) {
        this.isValueValidJMeasurements.push(j)
        this.isValueValid = 1
        this.errorMessageMeasurements = 'Invalid value. Only numbers are allowed.'
      } else {
        let formattedNumber = parseFloat(value);

        // Перевірка на NaN (нечислові значення)
        if (isNaN(formattedNumber)) {
          this.isValueValid = 1;
          this.isValueValidJMeasurements.push(j);
          this.errorMessageMeasurements = 'Invalid number format.';
          return;
        }

        // Округлення до 2 знаків
        let extractedNumber = formattedNumber.toFixed(2); // Зберігаємо як рядок
        console.log('extractedNumber after rounding:', extractedNumber); // Додано для дебагу
        this.metafields.value[group][ms] = extractedNumber// Оновлюємо значення

      }
    } else {
      this.removeValidationErrorMeasurements(j)
      this.isValueValid = -1
      this.metafields.value[group][ms] = value;
    }
  ...
  removeValidationError(i: number, j: number) {
    const indexToRemoveJ = this.isValueValidJ.indexOf(j);
    const indexToRemoveI = this.isValueValidI.indexOf(i);
    if (indexToRemoveJ !== -1) {
      this.isValueValidJ.splice(indexToRemoveJ, 1);
  ...
  updateFinalGarmentMetafieldTest(ev: any, group: any, ms: any,i: number, j: number, gender: string) {
    let val = ev.length > 0 ? ev : '0';
    let value = val.replace(',', '.');
    console.log('value',value)
    this.removeValidationError(i,j)
    if (value !== '0' || ms === 'front_waist_height' || ms === 'back_waist_height') {
      let objMinAndMax: any
      if (gender === 'Male') {
        objMinAndMax = this.measurementPlaceholderNumberAll[group][ms];
      } else {
        objMinAndMax = this.measurementPlaceholderFemaleOrOtherNumberAll[group][ms];
      }
      // Перевіряємо, чи є букви
      const hasLetters = /[a-zA-Z]/.test(value);
      if (hasLetters) {
        ...
        if (formattedNumber < objMinAndMax.min || (formattedNumber > objMinAndMax.max && formattedNumber !== objMinAndMax.max)) {
          this.isValueValid = 1
          this.isValueValidJ.push(j)
          this.isValueValidI.push(i)
          this.errorMessage = 'Invalid value. Must be in range'
        ...
        this.metafields.value.final_garment[group][ms] = extractedNumber// Оновлюємо значення

      }
    } else {
      this.removeValidationError(i,j)
      this.isValueValid = -1
      this.metafields.value.final_garment[group][ms] = value;
    }
    console.log(this.metafields.value.final_garment[group][ms]);
  }
  getFinalGarmentValue(type: string, ms: string): string {
    if (this.isEditMode) {
      return this.metafields.value.final_garment[type][ms] !== '0' ?
        this.metafields.value.final_garment[type][ms] : '';
    } else {
      return this.metafields.value.final_garment[type][ms] !== '0' ?
        this.metafields.value.final_garment[type][ms] + ' cm' : '';
    }
  }
  ...
  updateMetafieldsValue() {
    console.log(this.metafields.value.additional_info['images'])
...
/*  selectedInspirationPhoto(event: any) {
  ...
  updatePhotoDescription(ev: any, id: number) {
    this.metafields.value.additional_info['images'].fullBodyPhotos[id].description = ev.target.value;
    this.updateMetafieldsValue();
  }
